...
func display(fltr:Filter, idx=-1):
	filter = fltr
	or_check_box.pressed = filter.is_or_filter
	enabled_check_box.pressed = filter.enabled
	text_field.text = filter.string_value
	bool_check_box.pressed = filter.boolean_value
	inc_derivs.pressed = filter.include_derivatives
	inclusive_cb.pressed = filter.inclusive
